...
describe('GET /fees/history?blockCount={blockCount}?newestBlock={revision}', function () {
    it.e2eTest(
        ...
        async () => {
            ...
            const expectedRes = {
                baseFees: expect.arrayContaining([expect.stringMatching(HEX_AT_LEAST_1)]),
    ...
    revisions.valid(true).forEach((revision) => {
        it.e2eTest(`Fees history: valid revision ${revision}`, 'all', async () => {
